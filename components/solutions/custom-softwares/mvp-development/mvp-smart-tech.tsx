...
import { tech_choices } from "@/constant";
import { ArrowRight, CheckCircle } from "lucide-react";
import React, { useState } from "react";

function MvpSmartTech() {
  const [showAll, setShowAll] = useState(false);
  const displayedChoices = showAll ? tech_choices : tech_choices.slice(0, 3);
  const hasMoreChoices = tech_choices.length > 3;
  return (
    <div className="w-full md:py-32 py-20 bg-app-gray-bg">
      <section className="max-container 2xl:w-[85%] md:w-[95%] w-[90%] mx-auto">
        <div className="inter xl:pb-12 pb-6 flex flex-col gap-2">
          <Typography typo="header-3-light">Smart Tech Choices For</Typography>
          <Typography typo="header-3-semibold" className="!font-bold">
            Fast and Smooth MVP Development
          </Typography>
        </div>
