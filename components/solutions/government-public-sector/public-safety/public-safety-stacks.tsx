...
import { public_safety_stacks } from "@/constant";
import { ArrowRight, CheckCircle } from "lucide-react";
import React, { useState } from "react";

function PublicSafetyStacks() {
  const [showAll, setShowAll] = useState(false);
  const displayedChoices = showAll
    ? public_safety_stacks
    : public_safety_stacks.slice(0, 3);
  const hasMoreChoices = public_safety_stacks.length > 3;
  return (
...
export default PublicSafetyStacks;
