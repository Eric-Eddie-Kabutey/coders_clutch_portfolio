...
import { smart_education_stacks } from "@/constant";
import { ArrowRight, CheckCircle } from "lucide-react";
import React, { useState } from "react";

function SmartEducationStacks() {
  const [showAll, setShowAll] = useState(false);
  const displayedChoices = showAll
    ? smart_education_stacks
    : smart_education_stacks.slice(0, 3);
  const hasMoreChoices = smart_education_stacks.length > 3;
  return (
...
export default SmartEducationStacks;
